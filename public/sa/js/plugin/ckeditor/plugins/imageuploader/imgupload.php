...
$ckpath = "/public/sa/js/plugin/ckeditor/plugins/imageuploader/$useruploadpath";
...
if ($_FILES["upload"]["size"] > 5024000) {
    echo "<script>alert('".$uploadimgerrors3.$_FILES["upload"]["size"]."');</script>";
    $uploadOk = 0;
}
// Allow certain file formats
if(strtolower($imageFileType) != "jpg" && strtolower($imageFileType) != "png" && strtolower($imageFileType) != "jpeg"
&& strtolower($imageFileType) != "gif" && strtolower($imageFileType) != "ico" ) {
    echo "<script>alert('".$uploadimgerrors4 . $imageFileType ."');</script>";
    $uploadOk = 0;
}
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["upload"]["tmp_name"], $target_file)) {
        ...
        {
            echo "<script>alert('success');reloadImages();</script>";
        }
    } else {
